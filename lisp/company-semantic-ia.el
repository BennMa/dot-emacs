...
(defgroup company-semantic-ia nil
  "Completion backend using Semantic."
  :group 'company)

(defcustom company-semantic-ia-metadata-function 'company-semantic-ia-summary-and-doc
  "The function turning a semantic tag into doc information."
  :type 'function)

(defcustom company-semantic-ia-begin-after-member-access t
...
(defcustom company-semantic-ia-insert-arguments t
  "When non-nil, insert function arguments as a template after completion."
  :type 'boolean
  :package-version '(company . "0.9.0"))

(defvar company-semantic-ia-modes '(php-mode))

(defvar-local company-semantic-ia--current-tags nil
  "Tags for the current context.")

(defun company-semantic-ia-documentation-for-tag (tag)
...
(defun company-semantic-ia-doc-or-summary (tag)
  (or (company-semantic-ia-documentation-for-tag tag)
      (and (require 'semantic-idle nil t)
           (require 'semantic/idle nil t)
           (funcall semantic-idle-summary-function tag nil t))))

(defun company-semantic-ia-summary-and-doc (tag)
  (let ((doc (company-semantic-ia-documentation-for-tag tag))
...
(defun company-semantic-ia-doc-buffer (tag)
  (let ((doc (company-semantic-ia-documentation-for-tag tag)))
    (when doc
      (company-doc-buffer
...
(defsubst company-semantic-ia-completions (prefix)
  (ignore-errors
    (let ((completion-ignore-case nil)
          (context (semantic-analyze-current-context)))
      (setq company-semantic-ia--current-tags
            (semantic-analyze-possible-completions context 'no-unique))
      (all-completions prefix company-semantic-ia--current-tags))))

(defun company-semantic-ia-completions-raw (prefix)
  (setq company-semantic-ia--current-tags nil)
  (dolist (tag (semantic-analyze-find-tags-by-prefix prefix))
    (unless (eq (semantic-tag-class tag) 'include)
      (push tag company-semantic-ia--current-tags)))
  (delete "" (mapcar 'semantic-tag-name company-semantic-ia--current-tags)))

(defun company-semantic-ia-annotation (argument tags)
...
;;;###autoload
(defun company-semantic-ia (command &optional arg &rest ignored)
  "`company-mode' completion backend using CEDET Semantic."
  (interactive (list 'interactive))
  (cl-case command
    (interactive (company-begin-backend 'company-semantic-ia))
    (prefix (and (featurep 'semantic)
                 (semantic-active-p)
                 (memq major-mode company-semantic-ia-modes)
    ...
    (meta (funcall company-semantic-ia-metadata-function
                   (assoc arg company-semantic-ia--current-tags)))
    (annotation (company-semantic-ia-annotation arg
                                             company-semantic-ia--current-tags))
    (doc-buffer (company-semantic-ia-doc-buffer
                 (assoc arg company-semantic-ia--current-tags)))
    ;; Because "" is an empty context and doesn't return local variables.
    (no-cache (equal arg ""))
    (duplicates t)
    (location (let ((tag (assoc arg company-semantic-ia--current-tags)))
